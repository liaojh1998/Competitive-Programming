//Title: Square Root
//Type: Math
//Solution: Square Root Algorithm + High Precision Computation Algorithm (Directly same solution from SGU 111)
...
int main(){
	...
	while(C--){
		...
		if(C != 0)
			printf("\n");
		//Reset
		memset(sol.num, 0, sizeof(sol.num));
		memset(rem.num, 0, sizeof(rem.num));
		memset(bigtemp.num, 0, sizeof(bigtemp.num));
		memset(remtemp.num, 0, sizeof(remtemp.num));
		sol.len = rem.len = bigtemp.len = remtemp.len = start = 0;
	}
	return 0;
}