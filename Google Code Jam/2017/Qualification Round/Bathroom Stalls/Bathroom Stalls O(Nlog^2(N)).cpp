...
//Solution: This is not the actual solution, I'm simulating the process to figure out the pattern to solve this problem in O(1) time.
		//	This time uses heaps to create a faster solution.
#include <bits/stdc++.h>
using namespace std;
...
public:
	...
	void solve(){
		...
		for(int cases = 1; cases <= T; cases++){
			...
			readLL(N);
			readLL(K);
			//Observation 1: Longest segment is taken first, and divided in 2 where middle is (a + b)/2
			//Observation 2: Leftmost segment == Rightmost segment..., only the longest segment matters and output is all the same
			priority_queue<long long, vector<long long>, less<long long> > q;
			q.push(N);
			long long temp, maxs, mins;
			for(long long i = 1; i <= K; i++){
				temp = q.top(); q.pop();
				maxs = temp/2, mins = (temp-1)/2;
				printf("%lld %lld %lld\n", i, maxs, mins);
				q.push(maxs);
				q.push(mins);
			}
